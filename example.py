...
#             return {'data':values.HistGP, 'layout':layout}
...
def download_server(n):
    if n is not None:
        values.data = requests.get('http://127.0.0.1:5000/OptimizationData').json()
        print(values.data)
...
def update_graph(n):
    ...
    data_ecg = values.data['polar ECG']['data_ecg']
    if data_ecg is not None:
       values.ECGy = np.array(data_ecg).flatten()
